...
// Initial realistic transport routes for Nellore campus
...
export async function subscribeRouteAction(routeId: string, stopId: string) {
  ...
  return { success: true, message: "Route subscription confirmed. Fee updated in student ledger." };
}
...
export async function toggleBusOptOutAction(optedOut: boolean, reason?: string) {
  ...
  return {
    ...
    optedOut,
    message: optedOut
      ? "Driver manifest updated: Student marked as 'Parent Self-Pickup'. Bus will not wait."
      : "Bus pickup status restored to active for today.",
  };
}
...
export async function simulateBoardingScanAction(busNumber: string) {
  ...
  return {
    success: true,
    scannedAt: timestamp,
    message: `Digital Boarding Pass verified on ${busNumber} at ${timestamp}. WhatsApp 'Safe Ride' alert triggered to parent.`,
  };
}
